...
BlockController.prototype.getPoolInfo = function(block) {
  ...
  try {
    // This get an exeption in some block.
    // Get the output script and get the coinbase address from bitcore.
	  var script = block.transactions[0].outputs[0].script.chunks[2].buf; //[0];
	  var addr =   bitcore.Address(script,"livenet","pubkeyhash").toString();
	  var addrShort =  addr.substring(0, 10)+" ...";
	} catch(err) {console.log(err);}

  var poolInfo = {
    "poolName":  addr, // +'/n'+ coinbaseBuffer.toString('utf-8'),
    "url": "/address/"+addr
  };

  // return coinbase addresse only if nothing in the pools.json
  return poolInfo;
};
